...
export function createNotificationOptions(pushPayload: PushPayload, notificationInfo?: NotificationInfo): NotificationOptions {
    const { access_token, body, icon, notification_id, notification_type, preferred_locale } = pushPayload;
...
*/
    ...
    */
        if (notification.status) {
            ...
            if (notification.type === "favourite" || notification.type === "reblog" || notification.type === "mention") notificationOptions.data.url = `${user.server}/@${user.account.username}/${notification.status.id}`;

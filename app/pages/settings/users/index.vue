...
async function exportTokens() {
    ...
    if (!confirm("Please aware that the tokens represent the **full access** to your accounts, and should be treated as sensitive information. Are you sure you want to export the tokens?")) return;
...
async function importTokens() {
    ...
    try {
        ...
        for (const user of users) {
            if (loggedInUsers.value.some((u) => u.server === user.server && u.account.id === user.account.id)) continue;
            newUsers.push(user);
        }
        ...
        if (!confirm(`Found ${newUsers.length} new users, are you sure you want to import them?`)) return;

export default defineNuxtRouteMiddleware(async (to, from) => {
    ...
    try {
        ...
        if (to.params.account && to.name !== "status" && to.params.account.includes("@")) {
            ...
            if (account) return getAccountRoute(account);
        }
        ...
        if (statuses[0]) return getStatusRoute(statuses[0]);

        if (accounts[0]) return getAccountRoute(accounts[0]);
    } catch (err) {
        console.error(err);
    }

...
function groupItems(items: mastodon.v1.Notification[]): NotificationSlot[] {
    ...
    const processGroup = () => {
        ...
        if (group[0].type === "follow") {
            ...
            if (processedGroup.length > 0 && hasHeader(processedGroup[0].account)) results.push(processedGroup.shift()!);

            if (processedGroup.length === 1 && hasHeader(processedGroup[0].account)) results.push(processedGroup.shift()!);
...
function removeFiltered(items: mastodon.v1.Notification[]): mastodon.v1.Notification[] {
    return items.filter((item) => !item.status?.filtered?.find((filter) => filter.filter.filterAction === "hide" && filter.filter.context.includes("notifications")));
}

...
async function openEmojiPicker() {
    ...
    if (picker.value) {
        ...
        const [Picker, dataPromise, i18n] = await Promise.all([import("emoji-mart").then(({ Picker }) => Picker), import("@emoji-mart/data/sets/14/twitter.json").then((r: any) => r.default).catch(() => {}), importEmojiLang(locale.value.split("-")[0])]);

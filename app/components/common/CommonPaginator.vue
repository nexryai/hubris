...
const { items, prevItems, update, state, endAnchor, error } = usePaginator(
    paginator,
    toRef(() => stream),
    eventType,
    preprocess,
...
function updateEntry(item: any) {
    ...
    if (index > -1) items.value = [...items.value.slice(0, index), preprocess?.([item]) ?? item, ...items.value.slice(index + 1)];
}

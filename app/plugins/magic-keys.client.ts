...
export default defineNuxtPlugin(({ $scrollToTop }) => {
    ...
    const notUsingInput = computed(() => activeElement.value?.tagName !== 'INPUT' && activeElement.value?.tagName !== 'TEXTAREA' && !activeElement.value?.isContentEditable)
    ...
    if (!virtualScroller.value) {
        ...
        function getNextOrPreviousStatusId(currentStatusId: string | undefined, direction: 'next' | 'previous'): string | undefined {
            ...
            const statusId = direction === 'next' ? statusIds[Math.min(currentIndex + 1, statusIds.length)] : statusIds[Math.max(0, currentIndex - 1)]
            return statusId
        }

    ...
    useHydratedHead({
        ...
        titleTemplate: (title?: string) => {
            ...
            if (titleTemplate.match(/&[a-z0-9#]+;/gi)) {
                ...
                if (titleTemplate.length > 60) titleTemplate = `${titleTemplate.slice(0, 60)}...${titleTemplate.endsWith('"') ? '"' : ""}`;

                if (!titleTemplate.includes('"')) titleTemplate = `"${titleTemplate}"`;
            } else if (titleTemplate.length > 60) {
                titleTemplate = `${titleTemplate.slice(0, 60)}...${titleTemplate.endsWith('"') ? '"' : ""}`;
            }

            if (titleTemplate.length) titleTemplate += " | ";

            titleTemplate += t("app_name");
            if (buildInfo.env !== "release") titleTemplate += ` (${buildInfo.env})`;

            return titleTemplate;
        },
        link:
            import.meta.client && useAppConfig().pwaEnabled
                ? () => [
                      {
                          key: "webmanifest",
                          rel: "manifest",
...
function unescapeTitleTemplate(titleTemplate: string, replacements: [string, string[]][]) {
    ...
    for (const [replacement, entities] of replacements) {
        for (const e of entities) result = result.replaceAll(e, replacement);
    }
    return result.trim();
}

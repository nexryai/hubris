...
import { callOrReturn, InputRule, mergeAttributes, Node, nodePasteRule } from '@tiptap/core'
...
function createEmojiRule<NR extends typeof nodeInputRule | typeof nodePasteRule>(nodeRule: NR, type: Parameters<NR>[0]['type']): ReturnType<NR>[] {
    const rule = nodeRule({
...
export const TiptapPluginEmoji = Node.create({
    ...
    addInputRules() {
        function emojiInputRule(config: { find: InputRuleFinder, type: NodeType, getAttributes?: Record<string, any> | ((match: ExtendedRegExpMatchArray) => Record<string, any>) | false | null }) {
            return new InputRule({
                ...
                handler: ({ state, range, match }) => {
                    ...
                    tr.insert(start, config.type.create(attributes)).delete(tr.mapping.map(start), tr.mapping.map(end))

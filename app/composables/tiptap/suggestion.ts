...
export const TiptapEmojiSuggestion: Partial<SuggestionOptions> = {
    ...
    async items({ query }): Promise<(CustomEmoji | Emoji)[]> {
        ...
        const customEmojis: CustomEmoji[] = currentCustomEmojis.value.emojis.filter(emoji => emoji.shortcode.toLowerCase().startsWith(lowerCaseQuery)).map(emoji => ({ ...emoji, custom: true }))

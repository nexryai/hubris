...
export function useUserSettings() {
    ...
    if (settingsStorage.value.fontSize && !settingsStorage.value.fontSize.includes("px")) settingsStorage.value.fontSize = oldFontSizeMap[settingsStorage.value.fontSize as OldFontSize] as FontSize;
...
export function getPreferences<T extends keyof PreferencesSettings>(userSettings: UserSettings, name: T): PreferencesSettings[T] {
    ...
    if (name === "enableAutoplay") return getPreferences(userSettings, "enableDataSaving") ? false : preference;

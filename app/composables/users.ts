...
import { DEFAULT_POST_CHARS_LIMIT, STORAGE_KEY_CURRENT_USER_HANDLE, STORAGE_KEY_NODES, STORAGE_KEY_NOTIFICATION, STORAGE_KEY_NOTIFICATION_POLICY, STORAGE_KEY_SERVERS } from "~/constants";
    ...
    const [me, pushSubscription] = await Promise.all([
        ...
        useAppConfig().pwaEnabled
            ? // we get 404 response instead empty data
              client.value.v1.push.subscription
                  .fetch()
...
export async function fetchAccountInfo(client: mastodon.rest.Client, server: string) {
    ...
    const [account, preferences] = await Promise.all([client.v1.accounts.verifyCredentials(), fetchPrefs()]);
...
export async function signOut() {
    ...
    if (index !== -1) {
        ...
        if (!users.value.some((u, i) => u.server === currentUser.value!.server && i !== index)) delete instanceStorage.value[currentUser.value.server];
...
export function useUserLocalStorage<T extends object>(key: string, initial: () => T): Ref<T> {
    if (import.meta.server || process.test) return shallowRef(initial());

...
export function setCached(key: string, value: any, override = false) {
    if (override || !cache.has(key)) cache.set(key, value);
}
...
export function fetchAccountById(id?: string | null): Promise<mastodon.v1.Account | null> {
    ...
    const promise = useMastoClient()
        .v1.accounts.$select(id)
        .fetch()
        .then((r) => {
            if (r.acct && !r.acct.includes("@") && domain) r.acct = `${r.acct}@${domain}`;
...
export async function fetchAccountByHandle(acct: string): Promise<mastodon.v1.Account> {
    ...
    async function lookupAccount() {
        ...
        if (!isGotoSocial.value) {
            // TODO: GoToSocial will support this endpoint from 0.10.0
        ...
        if (account.acct && !account.acct.includes("@") && domain) account.acct = `${account.acct}@${domain}`;
        return account;
    }

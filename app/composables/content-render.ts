...
export function contentToVNode(content: string, options?: ContentParseOptions): VNode {
    ...
    if (options?.hideEmojis && textContents.length === 0) tree = parseMastodonHTML(content, { ...options, hideEmojis: false });
...
export function nodeToVNode(node: Node): VNode | string | null {
    ...
    if (node.name === "mention-group") return h(ContentMentionGroup, node.attributes, () => node.children.map(treeToVNode));

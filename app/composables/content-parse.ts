...
export function parseMastodonHTML(html: string, options: ContentParseOptions = {}) {
    const { markdown = true, replaceUnicodeEmoji = true, convertMentionLink = false, collapseMentionLink = false, hideEmojis = false, mentions, status, inReplyToStatus } = options;
    ...
    if (markdown) {
        ...
        html = html
            ...
            .replace(/>(```|~~~)(\w*)([\s\S]+?)\1/g, (_1, _2, lang: string, raw: string) => {
                const code = htmlToText(raw).replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/`/g, "&#96;").replace(/\*/g, "&ast;");
...
export function recursiveTreeToText(input: Node): string {
    if (input && input.children && input.children.length > 0) return input.children.map((n: Node) => recursiveTreeToText(n)).join("");
...
function sanitize(allowedElements: Record<string, AttrSanitizers>): Transform {
    return (node) => {
        ...
        if (!Object.prototype.hasOwnProperty.call(allowedElements, node.name)) return null;
...
function filterHref() {
    const LINK_PROTOCOLS = new Set(["http:", "https:", "dat:", "dweb:", "ipfs:", "ipns:", "ssb:", "gopher:", "xmpp:", "magnet:", "gemini:"]);
...
function addBdiParagraphs(node: Node) {
    if (node.name === "p" && !("dir" in node.attributes) && node.children?.length && node.children.length > 1) node.attributes.dir = "auto";
...
function transformParagraphs(node: Node): Node | Node[] {
    ...
    if (node.parent?.type === DOCUMENT_NODE && node.name === "p" && node.parent.children.at(-1) !== node) return [node, h("p")];

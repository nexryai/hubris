...
export function useHumanReadableNumber() {
    ...
    const fn = (num: number) => {
        return n(num, num < 10000 ? "smallCounting" : num < 1000000 ? "kiloCounting" : "millionCounting", locale.value);
    };
...
export function useFormattedDateTime(value: MaybeRefOrGetter<string | number | Date | undefined | null>, options: Intl.DateTimeFormatOptions = { dateStyle: "long", timeStyle: "medium" }) {
...
export function useFileSizeFormatter() {
    ...
    function formatFileSize(size: number) {
        return size >= megaByte ? formatters.value[0].format(size / megaByte) : formatters.value[1].format(size / 1024);
    }

...
import type { CreatePushNotification, PushNotificationPolicy, PushNotificationRequest, SubscriptionResult } from "~/composables/push-notifications/types";
import { STORAGE_KEY_NOTIFICATION, STORAGE_KEY_NOTIFICATION_POLICY } from "~/constants";

const supportsPushNotifications = typeof window !== "undefined" && "serviceWorker" in navigator && "PushManager" in window && "getKey" in PushSubscription.prototype;

export function usePushManager() {
    const { client } = useMasto();
    const isSubscribed = ref(false);
    const notificationPermission = ref<PermissionState | undefined>(Notification.permission === "denied" ? "denied" : Notification.permission === "granted" ? "granted" : Notification.permission === "default" ? "prompt" : undefined);
    ...
    const pushNotificationData = ref(createRawSettings(currentUser.value?.pushSubscription, configuredPolicy.value[currentUser.value?.account?.acct ?? ""]));
    const oldPushNotificationData = ref(createRawSettings(currentUser.value?.pushSubscription, configuredPolicy.value[currentUser.value?.account?.acct ?? ""]));
    const saveEnabled = computed(() => {
        const current = pushNotificationData.value;
        const previous = oldPushNotificationData.value;
        return current.favourite !== previous.favourite || current.reblog !== previous.reblog || current.mention !== previous.mention || current.follow !== previous.follow || current.poll !== previous.poll || current.policy !== previous.policy;
    });
    ...
    watch(
        ...
        (subscription) => {
            ...
            pushNotificationData.value = createRawSettings(subscription, configuredPolicy.value[currentUser.value?.account?.acct ?? ""]);
            oldPushNotificationData.value = createRawSettings(subscription, configuredPolicy.value[currentUser.value?.account?.acct ?? ""]);
        },
        { immediate: true, flush: "post" },
    );

    const subscribe = async (notificationData?: CreatePushNotification, policy?: mastodon.v1.WebPushSubscriptionPolicy, force?: boolean): Promise<SubscriptionResult> => {
    ...
    const saveSettings = async (policy?: mastodon.v1.WebPushSubscriptionPolicy) => {
        ...
        else configuredPolicy.value[currentUser.value!.account.acct ?? ""] = pushNotificationData.value.policy;
    ...
    const updateSubscription = async () => {
        if (currentUser.value) {
            ...
            else currentUser.value.pushSubscription = await client.value.v1.push.subscription.update({ data });
...
function createRawSettings(pushSubscription?: mastodon.v1.WebPushSubscription, subscriptionPolicy?: mastodon.v1.WebPushSubscriptionPolicy) {
    return {

...
export function useStatusActions(props: StatusActionsProps) {
    ...
    async function toggleStatusAction(action: Action, fetchNewStatus: () => Promise<mastodon.v1.Status>, countField?: CountField) {
        ...
        fetchNewStatus()
            .then((newStatus) => {
                ...
                if (isCancel && countField && prevCount === newStatus[countField]) newStatus[countField] -= 1;
    ...
    const canReblog = computed(() => status.value.visibility !== "direct" && (status.value.visibility !== "private" || status.value.account.id === currentUser.value?.account.id));
    ...
    const toggleFavourite = () => toggleStatusAction("favourited", () => client.value.v1.statuses.$select(status.value.id)[status.value.favourited ? "unfavourite" : "favourite"](), "favouritesCount");

    const toggleBookmark = () => toggleStatusAction("bookmarked", () => client.value.v1.statuses.$select(status.value.id)[status.value.bookmarked ? "unbookmark" : "bookmark"]());

    const togglePin = async () => toggleStatusAction("pinned", () => client.value.v1.statuses.$select(status.value.id)[status.value.pinned ? "unpin" : "pin"]());

    const toggleMute = async () => toggleStatusAction("muted", () => client.value.v1.statuses.$select(status.value.id)[status.value.muted ? "unmute" : "mute"]());

...
export const supportedTranslationCodes = ["ar", "az", "cs", "da", "de", "el", "en", "eo", "es", "fa", "fi", "fr", "ga", "he", "hi", "hu", "id", "it", "ja", "ko", "nl", "pl", "pt", "ru", "sk", "sv", "tr", "uk", "vi", "zh"] as const;
...
export async function translateText(text: string, from: string | null | undefined, to: string) {
    ...
    try {
        ...
        // TODO: improve type
        if ((err as TranslationErr).data?.error) status.value.error = (err as TranslationErr).data!.error!;
...
export async function useTranslation(status: mastodon.v1.Status | mastodon.v1.StatusEdit, to: string) {
    if (!translations.has(status)) translations.set(status, reactive({ visible: false, text: "", success: false, error: "" }));
    ...
    if ("language" in status) {
        ...
        if (!translationAPISupported) {
            shouldTranslate = shouldTranslate && supportedTranslationCodes.includes(to as any) && supportedTranslationCodes.includes(status.language as any);
        } else {
            shouldTranslate =
                shouldTranslate &&

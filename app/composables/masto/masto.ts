...
export function mastoLogin(masto: ElkMasto, user: Pick<UserLogin, "server" | "token">) {
    ...
    masto.client.value.v2.instance
        ...
        .then((newInstance) => {
            ...
            if (newInstance.configuration.urls.streaming !== streamingApiUrl) masto.streamingClient.value = createStreamingClient(newInstance.configuration.urls.streaming);
...
export function useStreaming(cb: (client: mastodon.streaming.Client) => mastodon.streaming.Subscription, options: UseStreamingOptions<true>): { stream: Ref<mastodon.streaming.Subscription | undefined> } & Pausable;
export function useStreaming(cb: (client: mastodon.streaming.Client) => mastodon.streaming.Subscription, options?: UseStreamingOptions<false>): Ref<mastodon.streaming.Subscription | undefined>;
export function useStreaming(cb: (client: mastodon.streaming.Client) => mastodon.streaming.Subscription, { immediate = true, controls }: UseStreamingOptions<boolean> = {}): ({ stream: Ref<mastodon.streaming.Subscription | undefined> } & Pausable) | Ref<mastodon.streaming.Subscription | undefined> {
    ...
    if (import.meta.client && !process.test) useNuxtApp().$pageLifecycle.addFrozenListener(cleanup);

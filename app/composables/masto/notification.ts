...
export function useNotifications() {
    ...
    async function processNotifications(stream: mastodon.streaming.Subscription, id: string) {
        for await (const entry of stream) {
            if (entry.event === "notification" && notifications[id]) notifications[id]![1].unshift(entry.payload.id);
        }
    }

    async function connect(): Promise<void> {
        if (!isHydrated.value || !id || notifications[id] !== undefined || !currentUser.value?.token) return;

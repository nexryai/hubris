...
export const currentUserDrafts = import.meta.server || process.test ? computed<DraftMap>(() => ({})) : useUserLocalStorage<DraftMap>(STORAGE_KEY_DRAFTS, () => ({}));

export const builtinDraftKeys = ["dialog", "home"];
...
export function getDefaultDraftItem(options: Partial<Mutable<mastodon.rest.v1.CreateStatusParams> & Omit<DraftItem, "params">> = {}): DraftItem {
    const { attachments = [], initialText = "", status, inReplyToId, visibility, sensitive, spoilerText, language, mentions, poll } = options;
...
export function useDraft(draftKey: string, initial: () => DraftItem = () => getDefaultDraftItem({})): UseDraft {

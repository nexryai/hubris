...
export function usePublish(options: { draftItem: Ref<DraftItem>; expanded: Ref<boolean>; isUploading: Ref<boolean>; isPartOfThread: boolean; initialDraft: () => DraftItem }) {
    ...
    const isPublishDisabled = computed(() => {
        ...
        return (
            ...
            (params.poll !== null &&
                ...
                ((firstEmptyInputIndex !== -1 && firstEmptyInputIndex !== params.poll.options.length - 1) ||
                    ...
                    (currentInstance.value?.configuration?.polls.maxCharactersPerOption !== undefined && params.poll.options.find((option) => option.length > currentInstance.value!.configuration!.polls.maxCharactersPerOption) !== undefined)))
        );
    });
    ...
    watch(
        draftItem,
        () => {
            if (failedMessages.value.length > 0) failedMessages.value.length = 0;
        },
        { deep: true },
    );
    ...
    async function publishDraft() {
        ...
        if (draftItem.value.mentions?.length) content = `${draftItem.value.mentions.map((i) => `@${i}`).join(" ")} ${content}`;
        ...
        if (draftItem.value.params.poll) {
            ...
            if (currentInstance.value?.configuration !== undefined && (options.length < currentInstance.value.configuration.polls.maxOptions || options[options.length - 1].trim().length === 0)) {
        ...
        try {
            ...
            if (draftItem.value.params.inReplyToId && !options.isPartOfThread) navigateToStatus({ status });
...
export function useUploadMediaAttachment(draft: Ref<DraftItem>) {
    ...
    const loadImage = (inputFile: Blob) =>
    ...
    async function processImageFile(file: File) {
        try {
            ...
            if (image.width * image.height > maxPixels.value) file = (await resizeImage(image, file.type)) as File;
            ...
            if (draft.value.attachments.length < limit) {
                if (file.type.startsWith("image/")) {
                    ...
                    if (maxVideoSize > 0 && file.size > maxVideoSize) {
                        const key = file.type.startsWith("audio/") ? "state.attachments_limit_audio_error" : file.type.startsWith("video/") ? "state.attachments_limit_video_error" : "state.attachments_limit_unknown_error";
                        const errorMessage = t(key, [formatFileSize(maxVideoSize)]);
                        failedAttachments.value = [...failedAttachments.value, [file.name, errorMessage]];
                        continue;
                    }
                }
    ...
    async function setDescription(att: mastodon.v1.MediaAttachment, description: string) {
        ...
        if (!draft.value.editingStatus) await client.value.v1.media.$select(att.id).update({ description: att.description });
    }
    ...
    async function onDrop(files: File[] | null) {
        if (files) await uploadAttachments(files);
    }

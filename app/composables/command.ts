...
const scopes = ['', 'Actions', 'Tabs', 'Navigation', 'Preferences', 'Account', 'Languages', 'Switch account', 'Settings', 'Hashtags', 'Users'] as const
    ...
    return {
        ...
        query: (scope: string, query: string): QueryResult => {
            ...
            if (query) {
                ...
                const grouped = new Map<CommandScopeNames, CommandQueryResultItem[]>(scopes.map(scope => [scope, []]))
...
export function useCommands(cmds: () => CommandProvider[]) {
    ...
    watch(
        commands,
        (n, o = []) => {
            for (const cmd of o) registry.remove(cmd)
            for (const cmd of n) registry.register(cmd)
        },

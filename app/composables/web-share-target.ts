export function useWebShareTarget(listener?: (message: MessageEvent) => void) {
    ...
    onBeforeMount(() => {
        ...
        if (useNuxtApp().$pwa?.isInstalled && "serviceWorker" in navigator) {
            if (listener) navigator.serviceWorker.addEventListener("message", listener);
            ...
            if (listener) onBeforeUnmount(() => navigator.serviceWorker.removeEventListener("message", listener));
        }
    });
}

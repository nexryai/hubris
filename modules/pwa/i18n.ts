...
export async function createI18n(): Promise<LocalizedWebManifest> {
    ...
    const locales: RequiredWebManifestEntry[] = await Promise.all(
        pwaLocales
            ...
            .map(async ({ code, dir = "ltr", file, files }) => {
                ...
                const { action, app_desc_short, app_name, nav, pwa } = file ? await readI18nFile(file as string) : await findBestWebManifestData(files as string[], env);
                const entry = pwa?.webmanifest?.[env] ?? {};

                if (!entry.name && app_name) entry.name = dir === "rtl" ? `${envName} ${app_name}` : `${app_name} ${envName}`;

                if (!entry.short_name && app_name) entry.short_name = dir === "rtl" ? `${envName} ${app_name}` : `${app_name} ${envName}`;
    ...
    return locales.reduce((acc, { lang, dir, name, short_name, description, shortcuts, screenshots }) => {
        acc[lang] = {
            lang,
            name,
            short_name,
...
async function readI18nFile(file: string) {
    ...
    return JSON.parse(Buffer.from(await readFile(resolve(`../../locales/${file}`), "utf-8")).toString());
}
...
async function findBestWebManifestData(files: string[], env: string) {
    ...
    for (const entry of entries) {
        ...
        if (entry.pwa?.screenshots) {
            ...
            Object.entries(entry.pwa.screenshots).forEach(([key, value]) => (pwa!.screenshots![key] = value));
        }
    }

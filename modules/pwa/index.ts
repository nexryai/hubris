...
export default defineNuxtModule<VitePWANuxtOptions>({
    ...
    async setup(options, nuxt) {
        ...
        nuxt.hook("vite:extend", ({ config }) => {
            ...
            if (plugin) throw new Error("Remove vite-plugin-pwa plugin from Vite Plugins entry in Nuxt config file!");
        });
        nuxt.hook("vite:extendConfig", async (viteInlineConfig, { isClient }) => {
            ...
            if (isClient) {
                viteInlineConfig.plugins.push({
                    ...
                    async writeBundle(_options, bundle) {
                        ...
                        for (const wm in webmanifests) await writeFile(join(manifestDir, `manifest-${wm}.webmanifest`), generateManifest(wm));
                    },
                });
            }
            viteInlineConfig.plugins.push({
                ...
                configureServer(server) {
                    ...
                    const mappedIcons: PwaDevIcon[] = [...icons.map(({ src, type }) => ({ src, type })), { src: "favicon.ico", type: "image/x-icon" }, { src: "apple-touch-icon.png", type: "image/png" }, { src: "logo.svg", type: "image/svg+xml" }];
            ...
            if (isClient) vitePwaClientPlugin = plugins.find((p) => p.name === "vite-plugin-pwa") as Plugin;
        });

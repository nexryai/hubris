...
export function configurePWAOptions(options: Partial<VitePWAOptions>, nuxt: Nuxt) {
    ...
    let config: Partial<import("workbox-build").BasePartial & import("workbox-build").GlobPartial & import("workbox-build").RequiredGlobDirectoryPartial>;
    ...
    if (options.strategies === "injectManifest") {
        ...
        if (nuxt.options.dev) {
            ...
            if (options.devOptions?.enabled && !options.devOptions.navigateFallbackAllowlist) options.devOptions.navigateFallbackAllowlist = [new RegExp(nuxt.options.app.baseURL) ?? /\//];
        }
    ...
    if (!("dontCacheBustURLsMatching" in config)) config.dontCacheBustURLsMatching = new RegExp(buildAssetsDir);
    ...
    if (!nuxt.options.dev) config.manifestTransforms = [createManifestTransform(nuxt.options.app.baseURL ?? "/", appManifestFolder)];
}

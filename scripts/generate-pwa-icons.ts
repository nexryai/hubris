...
async function optimizePng(filePath: string, png: PngOptions) {
    await sharp(filePath)
        .png(png)
        .toFile(`${filePath.replace(/-temp\.png$/, ".png")}`);
    await rm(filePath);
}
                ...
                .composite([
                    {
                        input: await sharp(svgLogo)
                            .resize(Math.round(size * (1 - padding)), Math.round(size * (1 - padding)), resizeOptions)
                            .toBuffer(),
                    },
                ])
                .toFile(filePath);
...
async function generatePWAIconForEnv(folder: string, icons: ResolvedIcons) {
    ...
    await Promise.all([generateTransparentIcons(icons, svgLogo, folder), generateMaskableIcons("maskable", icons, svgLogo, folder), generateMaskableIcons("apple", icons, svgLogo, folder)]);

    if (icons.ico) {
        const { icoName = (size) => `favicon-${size}x${size}.ico` } = icons.ico;
        await Promise.all(
            icons.ico.sizes.map(async (size) => {
                const png = await sharp(resolve(folder, icons.iconName("transparent", size).replace(/-temp\.png$/, ".png")))
                    .toFormat("png")
                    .toBuffer();
...
async function generatePWAIcons(folders: string[], icons: Icons) {
    ...
    if (!transparent.resizeOptions) transparent.resizeOptions = { ...defaultIcons.transparent.resizeOptions };

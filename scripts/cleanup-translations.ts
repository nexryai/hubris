...
for (const file of sourceFiles) {
    const data = JSON.parse(Buffer.from(await readFile(resolver.resolve(`../locales/${file as string}`), "utf-8")).toString()) as Record<string, unknown>;
    ...
    for (const locale of currentLocales.filter((l) => l.code !== "en-US")) {
        ...
        for (const file of files) {
            ...
            for (const key in targetTranslations) {
                if (!sourceTranslations[key]) delete targetTranslations[key];
            }

...
async function readI18nFile(file: string | string[]) {
    ...
    if (Array.isArray(file)) {
        const files = await Promise.all(
            file.map((f) => async () => {
                return JSON.parse(Buffer.from(await readFile(resolver.resolve(`../locales/${f}`), "utf-8")).toString());
            }),
        ).then((f) => f.map((f) => f()));
        ...
        return JSON.parse(Buffer.from(await readFile(resolver.resolve(`../locales/${file}`), "utf-8")).toString());
    }
}

async function compare(baseEntries: Record<string, string>, file: string | string[], data: LocaleEntry) {
...
async function prepareTranslationStatus() {
    ...
    Object.keys(data)
        .filter((k) => k !== "en")
        .forEach((e) => {
            const percentage = total <= 0.0 || data[e].total === 0.0 ? "0" : data[e].total === total ? "100" : ((data[e].translated.length / total) * 100).toFixed(1);
    ...
    await writeFile(resolver.resolve("../elk-translation-status.json"), JSON.stringify(translationStatus, null, 2), { encoding: "utf-8" });
}
